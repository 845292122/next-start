...
/**
 * `--app-font-serif` is not part of the vendored shadcn theme — it's this app's
 * own token, read by LoginHero and LoginForm for the sign-in screen's display
 * face. Mantine's own Latin stack has no CJK glyphs, hence the fallbacks.
 */
...
export const cssVariablesResolver: CSSVariablesResolver = () => ({
	variables: {
		'--app-font-serif': FONT_FAMILY_SERIF,
	...
	light: {
		...
		/*
		 * Measured with axe (e2e/a11y.e2e.ts): `secondary-10` (zinc's 11th, "brand
		 * accent" shade) is only ~4.4:1 against the SegmentedControl's tinted
		 * background — under the 4.5:1 AA floor for body text. `secondary-6` clears
		 * it with margin (~9.5:1) while staying visually distinct from body text.
		 */
		'--mantine-color-dimmed': 'var(--mantine-color-secondary-6)', // used as dimmed text color
		...
		// red-5's white-on-filled contrast measures ~4.4:1 (axe); red-6 clears AA.
		...
		// teal-5's white-on-filled contrast measures ~3.5:1 (axe); teal-6 clears AA.

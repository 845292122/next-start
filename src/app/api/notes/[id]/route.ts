...
/**
 * See ../route.ts on why there's no explicit 401 or 400 handling here, and on why
 * these handlers exist at all now that nothing in the app calls them.
 *
...
export const PATCH = withHandler<NoteRouteContext>(
	async (_request, { params }) => {
		...
		const { id } = await readParams(params, noteIdSchema)
